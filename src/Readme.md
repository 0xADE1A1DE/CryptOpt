...
- `modules`: git-submodules. Currenlty contains [0xAde1a1de/MeasureSuite](https://github.com/0xADE1A1DE/MeasureSuite).
- `src`: source files for CryptOpt
- `src/bridge`: We call a class, which provides a `JSON`-format of the input function and generates machinecode a **Bridge**. Those are in this folder, currently supported are `fiat-bridge` and `bitcoin-core-bridge`. Additionally an experimental `manual-bridge`, where one can specify a C and JSON source file to run the optimization on.
...
Output is then written to the `results/<CURVE>/<METHOD>/`, can be changes with the `--resultDir` option.
Output for each optmimization, it generates 
TBD
----------------------------------------------------------
