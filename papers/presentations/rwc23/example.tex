\begin{tikzpicture}[-latex, yscale=.7,xscale=.5, line width=.75pt]
    ...
    \only<14->   \node[above right=-.3 and .9 of code.north east, onslide=<15-17>{highlight}, text width=4em, align=center] {reorder};
    \only<14->   \node[below right=-.3 and .9 of code.south east, onslide=<20-22>{highlight}, text width=4em, align=center] {\texttt{template}\newline change};
    ...
    \pause
    \node [operation,                   \colA] (A1) at (15, 6) {$\fadd$};
