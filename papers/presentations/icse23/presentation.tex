...
\title{\cryptopt}
\subtitle{Automatic Optimization of Straightline Code\vspace{-2em}}
...
\begin{frame}
    ...
    \onslide<4->{
        \begin{itemize}
            ...
            \item<5->[2.] Prove it correct. ($\rightarrow$ in Full Paper, PLDI'23, Distinguished Paper)
        \end{itemize}
    }
\end{frame}
...
\begin{frame}[c]{}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            \item Compilation of straightline code \\$\implies$ Search \pause
            \item Random Local Search + Runtime \pause %to find the best implementation code
            \item \alert<4>{Proven-correct} assembly for field arithmetic by \fiat now with \alert<4>{on-par performance} to hand-optimized assembly. (see full Paper)\pause
        \end{itemize}
        ...
        \pause
        GitHub Project \\(with links to papers)
